...
public class LessonsController {
    ...
    public ResponseEntity<List<LessonModel>> findAllLessonsIntoModule(@PathVariable UUID moduleId){
        moduleService.findById(moduleId);
        List<LessonModel> list = lessonService.findAllLessonsIntoModule(moduleId);
    ...
    public ResponseEntity<LessonModel> saveLesson(@PathVariable UUID moduleId, @RequestBody @Valid LessonDto lessonDto){
        ...
        URI uri = ServletUriComponentsBuilder.fromCurrentRequest().path("/{lessonId}").buildAndExpand(obj.getLessonId()).toUri();
